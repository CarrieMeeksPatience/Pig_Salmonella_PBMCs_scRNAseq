...
# change cluster_prop_time$cluster = D0 to 0 DPI
  cluster_prop_time$DPI<-gsub("D0", "0 DPI", cluster_prop_time$DPI)
# change cluster_prop_time$cluster = D2 to 2 DPI
  cluster_prop_time$DPI<-gsub("D2", "2 DPI", cluster_prop_time$DPI)
# change cluster_prop_time$cluster = D8 to 8 DPI
  cluster_prop_time$DPI<-gsub("D8", "8 DPI", cluster_prop_time$DPI)
...
plot1<-ggplot(cluster_prop_time, aes(x = cluster, y = Proportion, fill = DPI)) +
  ...
  labs(title = "Proportions of cells across the whole dataset",
       x = "Cluster ID",
       y = "Proportion of the whole dataset",
       fill = "Days Post Inoculation:") +
...
# change cluster_prop_Animal$Animal = "A1" to "Animal 1"
cluster_prop_Animal$Animal<-gsub("852", "pig 852", cluster_prop_Animal$Animal)
cluster_prop_Animal$Animal<-gsub("854", "pig 854", cluster_prop_Animal$Animal)
cluster_prop_Animal$Animal<-gsub("864", "pig 864", cluster_prop_Animal$Animal)
cluster_prop_Animal$Animal<-gsub("842", "pig 842", cluster_prop_Animal$Animal)
cluster_prop_Animal$Animal<-gsub("853", "pig 853", cluster_prop_Animal$Animal)
...
plot1<-ggplot(cluster_prop_Animal,  aes(x = SubCluster, y = Proportion, fill = Animal)) +
  ...
  labs(title = "Proportions of cells from animal within SubCluster",
       x = "SubCluster",
       y = "Proportion of each SubCluster",
       fill = "Pig ID:") +
  theme(axis.title = element_text(size = 20, face = "bold", color = "black"), 
        axis.text.x = element_text(hjust = 0.5,vjust = 0.5,color = "black",size = 25,face = "bold"),
        axis.text.y = element_text(color = "black",size = 25,face = "bold"),
...
celltype_Animal_time <- rbind(
  celltype_Animal_time,
  "Total Cells in Sample" = colSums(celltype_Animal_time)
)
head(celltype_Animal_time)
celltype_Animal_time_df <- as.data.frame(celltype_Animal_time)
#pivot_longer
celltype_Animal_time_long <- celltype_Animal_time |>
...
head(celltype_Animal_time_long)
write.table(celltype_Animal_time_long, file="/scRNAseq/Sal_5pigs_2026/clustering_annotation/Block_cycling_genes/5pigs_salmonella_STACAS_via_Animal_ScranNorm_Regressed_PCA_d10_clustered_celltype_Animal_Number_Prop_long.txt", sep="\t", quote=F, row.names = FALSE)
...
Idents(All3_integrated)="celltypes_subclustres02"
celltype_sample_prop<-prop.table(table(Idents(All3_integrated), All3_integrated$Animal_time), margin = 2)
write.table(celltype_sample_prop, file="/scRNAseq/Sal_5pigs_2026/clustering_annotation/Block_cycling_genes/5pigs_salmonella_STACAS_via_Animal_ScranNorm_Regressed_PCA_d10_clustered_celltype_Sample100_Prop.txt", sep="\t", quote=F)
celltype_sample_prop <- rbind(
  celltype_sample_prop,
  "Total Cells in Sample" = colSums(celltype_sample_prop)
)
celltype_sample_prop_df <- as.data.frame(celltype_sample_prop)
#pivot_longer
celltype_sample_prop_long <- celltype_sample_prop |>
  as.data.frame.matrix() |>
  tibble::rownames_to_column(var = "Celltype") |>
  tidyr::pivot_longer(
    cols = -Celltype,
...
head(celltype_sample_prop_long)
write.table(celltype_sample_prop_long, file="/scRNAseq/Sal_5pigs_2026/clustering_annotation/Block_cycling_genes/5pigs_salmonella_STACAS_via_Animal_ScranNorm_Regressed_PCA_d10_clustered_celltype_Sample100_Prop_long.txt", sep="\t", quote=F, row.names = FALSE)
